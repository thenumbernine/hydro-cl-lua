...
TFBar(K_ij) = K_ij - 1/3 gammaTilde_ij gammaTilde^kl K_kl 
...
<? if eqn.guiVars.constrain_tr_ATilde_ll.value then ?>
	U->ATilde_ll = tracefree(U->ATilde_ll, U->gammaTilde_ll, U->gammaTilde_uu);
<? end ?>
}
...
<?=makePartial('gammaTilde_uu', 'sym3')?>	//partial_gammaTilde_uul[k].ij = gamma^ij_,k
<? else ?>
...
<?=makePartial('gammaTilde_ll', 'sym3')?>	//partial_gammaTilde[k].ij := gammaTilde_ij,k
...
<? end ?>
<?=makePartial2('gammaTilde_ll', 'sym3')?>	//partial2_gammaTilde_llll[kl].ij = gammaTilde_ij,kl
...
?>	connBar_lll.<?=xi?>.<?=xjk?> = .5 * (
		partial_gammaTilde_lll[<?=k-1?>].<?=sym(i,j)?> 
		+ partial_gammaTilde_lll[<?=j-1?>].<?=sym(i,k)?> 
		- partial_gammaTilde_lll[<?=i-1?>].<?=xjk?>);
<?	end
end
?>	
	//connBar_ull[i].jk := connBar^i_jk = gammaTilde^il connBar_ljk
	_3sym3 connBar_ull = sym3_3sym3_mul(U->gammaTilde_uu, connBar_lll);	

	//from 2006 Campanelli "connBar^i is replaced by -gammaTilde^ij_j wherever it is not differentiated"
...
?>
	//DBar^i phi = gammaTilde^ij phi_,j
	real3 DBar_phi_u = sym3_real3_mul(U->gammaTilde_uu, *(real3*)partial_phi_l);

	//conn_ull[i].jk := conn^i_jk
	//Alcubierre 2.8.14:
	//conn^i_jk = connBar^i_jk + 2 (delta^i_j phi_,k + delta^i_k phi_,j - gamma_jk gamma^il phi_,l)
	//B&S 3.7:
	//conn^i_jk = connBar^i_jk + 2 (delta^i_j phi_,k + delta^i_k phi_,j - gammaTilde_jk gammaTilde^il phi_,l)
	//conn^i_jk = connBar^i_jk + 2 (delta^i_j phi_,k + delta^i_k phi_,j - gammaTilde_jk DBar^i phi)
	_3sym3 conn_ull;	
<? 
for i,xi in ipairs(xNames) do
	for jk,xjk in ipairs(symNames) do
		local j,k = from6to3x3(jk)
?>	conn_ull.<?=xi?>.<?=xjk?> = connBar_ull.<?=xi?>.<?=xjk?> - 2 * U->gammaTilde_ll.<?=xjk?> * DBar_phi_u.<?=xi?><?
		if i==j then
?> + 2 * partial_phi_l[<?=k-1?>]<?
		end
		if i==k then
...
if eqn.useChi then
?>	
	real partial_chi_bardot_partial_alpha = real3_weightedDot( *(real3*)partial_chi_l, *(real3*)partial_alpha_l, U->gammaTilde_uu);
	
	//chi D_i D_j alpha = alpha,ij - connBar^k_ij alpha,k + 1/(2 chi) (alpha,i chi,j + alpha,j chi,i - gammaTilde_ij gammaTilde^kl alpha,k chi,l)
...
?>		.<?=xij?> = U->chi * (partial2_alpha_ll[<?=ij-1?>] <?
		for k,xk in ipairs(xNames) do
			...
			?> + .5 * partial_alpha_l[<?=i-1?>] * partial_chi_l[<?=j-1?>] <?
			?> - .5 * U->gammaTilde_ll.<?=xij?> * partial_chi_bardot_partial_alpha,
<? 	end
?>	};
	...
	real tr_D2_alpha = U->chi * (
		...
		- .5 * real3_weightedDot(
			...
			*(real3*)partial_alpha_l,
			U->gammaTilde_uu);
<? 	else 
?>	real tr_D2_alpha = sym3_dot(U->gammaTilde_uu, chi_D2_alpha_ll);
...
<? for ij,xij in ipairs(symNames) do
	local i,j = from6to3x3(ij)
	local xi = xNames[i]
?>	deriv->gammaTilde_ll.<?=xij?> += -2 * U->alpha * U->ATilde_ll.<?=xij?>	//-2 alpha ATilde_ij 
<? 	for k,xk in ipairs(xNames) do
?>		+ partial_gammaTilde_lll[<?=k-1?>].<?=xij?> * U->beta_u.<?=xk?>		//+ beta^k gammaTilde_ij,k 
		+ U->gammaTilde_ll.<?=sym(k,j)?> * partial_beta_ul[<?=i-1?>].<?=xk?>	//+ gammaTilde_jk beta^k_,i 
		+ U->gammaTilde_ll.<?=sym(k,i)?> * partial_beta_ul[<?=j-1?>].<?=xk?> 	//+ gammaTilde_ik beta^k_,j
<? 	end
...
?>
	...
	//tr_partial2_gammaTilde_ll.ij = gammaTilde^kl gammaTilde_ij,kl
	sym3 tr_partial2_gammaTilde_ll;	
<? for ij,xij in ipairs(symNames) do
?>	tr_partial2_gammaTilde_ll.<?=xij?> = 0. <?
	for k,xk in ipairs(xNames) do
		for l,xl in ipairs(xNames) do
?> + U->gammaTilde_uu.<?=sym(k,l)?> * partial2_gammaTilde_llll[<?=from3x3to6(k,l)-1?>].<?=xij?><?
		end
	end
?>;
<? end
?>
	...
	//RBar_ij = -1/2 gammaTilde^lm gammaTilde_ij,lm 
	//		+ 1/2 gammaTilde_ki connBar^k_,j
	//		+ 1/2 gammaTilde_kj connBar^k_,i 
...
?>	RBar_ll.<?=xij?> = -.5 * tr_partial2_gammaTilde_ll.<?=xij?>
<?	for k,xk in ipairs(xNames) do
?>		+ .5 * U->gammaTilde_ll.<?=sym(k,i)?> * partial_connBar_ul[<?=j-1?>].<?=xk?>
		+ .5 * U->gammaTilde_ll.<?=sym(k,j)?> * partial_connBar_ul[<?=i-1?>].<?=xk?>
		+ .5 * connBar_u.<?=xk?> * (connBar_lll.<?=xi?>.<?=sym(j,k)?> + connBar_lll.<?=xj?>.<?=sym(i,k)?>)
<?		for l,xl in ipairs(xNames) do
			for m,xm in ipairs(xNames) do
?>		+ U->gammaTilde_uu.<?=sym(k,m)?> * (
...
<? end
?>
	real tr_DBar2_phi = sym3_dot(U->gammaTilde_uu, DBar2_phi_ll);
	real DBar_phi_norm = real3_dot(*(real3*)partial_phi_l, DBar_phi_u);

	//Baumgarte & Shapiro p.57 eqn 3.10
	//R_ll(i,j) := R_ij = RBar_ij - 2 (DBar_i DBar_j ln(psi) + gammaTilde_ij gammaTilde^lm DBar_l DBar_m ln(psi)) + 4((DBar_i ln(psi)) (DBar_j ln(psi)) - gammaTilde_ij gammaTilde^lm (DBar_l ln(psi)) (DBar_m ln(psi)))
...
?>	RPhi_ll.<?=xij?> = 2. * (
		- DBar2_phi_ll.<?=xij?> 
		- U->gammaTilde_uu.<?=xij?> * tr_DBar2_phi 
		+ 2. * (partial_phi_l[<?=i-1?>] * partial_phi_l[<?=j-1?>] 
			- U->gammaTilde_ll.<?=xij?> * DBar_phi_norm));
<? end 
?>
...
?>
	...
	//		- 8 pi gammaTilde^ij S_j 
	//		+ 6 ATilde^ij phi_,j
	//	)
	//	+ beta^j connBar^i_,j
	//	- connBar^j beta^i_,j
	//	+ 2/3 connBar^i beta^j_,j
	//	+ 1/3 gammaTilde^ki beta^j_,jk
	//	+ gammaTilde^kj beta^i_,jk
	real3 dt_connBar_u;
<? for i,xi in ipairs(xNames) do
?>	dt_connBar_u.<?=xi?> =
...
?>		- 2. * ATilde_uu.<?=xij?> * partial_alpha_l[<?=j-1?>]
		+ 2. * U->alpha * (
			-2./3. * U->gammaTilde_uu.<?=xij?> * partial_K_l[<?=j-1?>] 
...
?>		+ 2. * U->alpha * connBar_ull.<?=xi?>.<?=xjk?> * ATilde_uu.<?=xjk?>
		+ 1./3. * U->gammaTilde_uu.<?=xik?> * partial2_beta_ull[<?=jk-1?>].<?=xj?>
		+ U->gammaTilde_uu.<?=xjk?> * partial2_beta_ull[<?=jk-1?>].<?=xi?>
<?		end
	end
?>	;
<? end
...
<? if calcConstraints then ?>
...
#if 0
	real RBar = sym3_dot(U->gammaTilde_uu, RBar_ll);
	...
	//
	//gammaTilde^ij DBar_i DBar_j exp(phi)
	// = exp(phi) (gammaTilde^ij DBar_i DBar_j phi + gammaTilde^ij phi_,i phi_,j)
	// = exp(phi) (tr_DBar2_phi + DBar_phi_norm)

	//H = gammaTilde^ij DBar_i DBar_j e^phi 
	//		- 1/8 e^phi RBar 
	...
	//= exp(6 phi) (6 ATilde^ij phi_,j + (gammaTilde^ik ATilde_kl gammaTilde^lj)_,j + connBar^i_jk ATilde^jk) ... plus (ln det gammaTilde_ll)_,k which is zero, right?
	//= exp(6 phi) (
	//		+ 6 ATilde^ij phi_,j
	//		+ gammaTilde^ik_,j ATilde_k^j
	//		+ ATilde^i_l gammaTilde^lj_,j
	//		+ gammaTilde^ik ATilde_kl,j gammaTilde^lj
	//		+ connBar^i_jk ATilde^jk)
   	//= exp(6 phi) (
	//		+ 6 ATilde^ij phi_,j
	//		- ATilde^kj gammaTilde^il gammaTilde_lk,j
	//		- ATilde^ik gammaTilde^mj gammaTilde_km,j
	//		+ gammaTilde^ik gammaTilde^lj ATilde_kl,j
	//		+ connBar^i_jk ATilde^jk)
