...
<? if true then ?>\
	...
	{\
		real const tmp1 = dHat_lll.x.yy * alpha;\
		real const tmp2 = dHat_lll.y.xy * alpha;\
		real const tmp3 = gamma_uu.yy * tmp2;\
		real const tmp4 = dDelta_lll.x.yy * alpha;\
		real const tmp5 = dDelta_lll.y.xy * alpha;\
		real const tmp6 = gamma_uu.yy * tmp5;\
		real const tmp7 = dHat_lll.y.xz * alpha;\
		real const tmp8 = gamma_uu.yz * tmp7;\
		real const tmp9 = dHat_lll.z.xy * alpha;\
		real const tmp10 = gamma_uu.yz * tmp9;\
		real const tmp11 = dDelta_lll.y.xz * alpha;\
		real const tmp12 = gamma_uu.yz * tmp11;\
		real const tmp13 = dDelta_lll.z.xy * alpha;\
		real const tmp14 = gamma_uu.yz * tmp13;\
		real const tmp15 = dHat_lll.x.zz * alpha;\
		real const tmp16 = dHat_lll.z.xz * alpha;\
		real const tmp17 = gamma_uu.zz * tmp16;\
		real const tmp18 = dDelta_lll.x.zz * alpha;\
		real const tmp19 = dDelta_lll.z.xz * alpha;\
		real const tmp20 = gamma_uu.zz * tmp19;\
		real const tmp21 = dHat_lll.x.yz * alpha;\
		real const tmp22 = gamma_uu.yz * tmp21;\
		real const tmp23 = dDelta_lll.x.yz * alpha;\
		real const tmp24 = gamma_uu.yz * tmp23;\
		real const tmp25 = gamma_uu.zz * tmp18;\
		real const tmp26 = gamma_uu.zz * tmp15;\
		real const tmp27 = gamma_uu.yy * tmp4;\
		real const tmp28 = gamma_uu.yy * tmp1;\
		real const tmp29 = 1. / 2.;\
		real const tmp30 = gamma_uu.xz * tmp7;\
		real const tmp31 = gamma_uu.xz * tmp9;\
		real const tmp32 = gamma_uu.xz * tmp11;\
		real const tmp33 = gamma_uu.xz * tmp13;\
		real const tmp34 = dHat_lll.y.yz * alpha;\
		real const tmp35 = gamma_uu.yz * tmp34;\
		real const tmp36 = dHat_lll.z.yy * alpha;\
		real const tmp37 = gamma_uu.yz * tmp36;\
		real const tmp38 = dDelta_lll.y.yz * alpha;\
		real const tmp39 = gamma_uu.yz * tmp38;\
		real const tmp40 = dDelta_lll.z.yy * alpha;\
		real const tmp41 = gamma_uu.yz * tmp40;\
		real const tmp42 = dHat_lll.y.zz * alpha;\
		real const tmp43 = gamma_uu.zz * tmp42;\
		real const tmp44 = dHat_lll.z.yz * alpha;\
		real const tmp45 = gamma_uu.zz * tmp44;\
		real const tmp46 = dDelta_lll.y.zz * alpha;\
		real const tmp47 = gamma_uu.zz * tmp46;\
		real const tmp48 = dDelta_lll.z.yz * alpha;\
		real const tmp49 = gamma_uu.zz * tmp48;\
		real const tmp50 = gamma_uu.xz * tmp21;\
		real const tmp51 = gamma_uu.xz * tmp23;\
		real const tmp52 = gamma_uu.yy * tmp34;\
		real const tmp53 = gamma_uu.yy * tmp36;\
		real const tmp54 = gamma_uu.yy * tmp38;\
		real const tmp55 = gamma_uu.yy * tmp40;\
		real const tmp56 = gamma_uu.yz * tmp42;\
		real const tmp57 = gamma_uu.yz * tmp44;\
		real const tmp58 = gamma_uu.yz * tmp46;\
		real const tmp59 = gamma_uu.yz * tmp48;\
		real const tmp60 = gamma_uu.xx * alpha;\
		real const tmp61 = gamma_uu.xy * alpha;\
		real const tmp62 = gamma_uu.xz * alpha;\
		real const tmp63 = gamma_uu.xy * gamma_uu.xy;\
		real const tmp64 = alpha * tmp63;\
		real const tmp65 = gamma_uu.xz * gamma_uu.xz;\
		real const tmp66 = alpha * tmp65;\
		...
		(resultFlux)->d_lll.z.yz = 0.;\
		(resultFlux)->d_lll.z.zz = 0.;\
		(resultFlux)->K_ll.xx = a_l.x * alpha + -2. * Z_l.x * alpha + -1. * tmp3 + -1. * tmp6 + -1. * tmp8 + -1. * tmp10 + -1. * tmp12 + -1. * tmp14 + -1. * tmp17 + -1. * tmp20 + 2. * tmp24 + 2. * tmp22 + tmp25 + tmp26 + tmp27 + tmp28;\
		(resultFlux)->K_ll.xy = a_l.y * alpha * tmp29 + -1. * Z_l.y * alpha + tmp29 * tmp30 + tmp29 * tmp31 + tmp29 * tmp32 + tmp29 * tmp33 + tmp29 * tmp35 + -1. * tmp29 * tmp37 + tmp29 * tmp39 + -1. * tmp29 * tmp41 + tmp29 * tmp43 + -1. * tmp29 * tmp45 + tmp29 * tmp47 + -1. * tmp29 * tmp49 + -1. * gamma_uu.xy * tmp1 + gamma_uu.xy * tmp2 + -1. * gamma_uu.xy * tmp4 + gamma_uu.xy * tmp5 + -1. * tmp51 + -1. * tmp50;\
		(resultFlux)->K_ll.xz = a_l.z * alpha * tmp29 + -1. * Z_l.z * alpha + gamma_uu.xy * tmp7 * tmp29 + gamma_uu.xy * tmp9 * tmp29 + gamma_uu.xy * tmp11 * tmp29 + gamma_uu.xy * tmp13 * tmp29 + -1. * tmp29 * tmp52 + tmp29 * tmp53 + -1. * tmp29 * tmp54 + tmp29 * tmp55 + -1. * tmp29 * tmp56 + tmp29 * tmp57 + -1. * tmp29 * tmp58 + tmp29 * tmp59 + -1. * gamma_uu.xy * tmp21 + -1. * gamma_uu.xy * tmp23 + -1. * gamma_uu.xz * tmp15 + gamma_uu.xz * tmp16 + gamma_uu.xz * tmp19 + -1. * gamma_uu.xz * tmp18;\
		(resultFlux)->K_ll.yy = gamma_uu.xx * tmp1 + -1. * gamma_uu.xx * tmp2 + gamma_uu.xx * tmp4 + -1. * gamma_uu.xx * tmp5 + -1. * gamma_uu.xz * tmp34 + gamma_uu.xz * tmp36 + gamma_uu.xz * tmp40 + -1. * gamma_uu.xz * tmp38;\
		(resultFlux)->K_ll.yz = -1. * gamma_uu.xx * tmp7 * tmp29 + -1. * gamma_uu.xx * tmp9 * tmp29 + -1. * gamma_uu.xx * tmp11 * tmp29 + -1. * gamma_uu.xx * tmp13 * tmp29 + gamma_uu.xy * tmp29 * tmp34 + -1. * gamma_uu.xy * tmp29 * tmp36 + gamma_uu.xy * tmp29 * tmp38 + -1. * gamma_uu.xy * tmp29 * tmp40 + -1. * gamma_uu.xz * tmp29 * tmp42 + gamma_uu.xz * tmp29 * tmp44 + -1. * gamma_uu.xz * tmp29 * tmp46 + gamma_uu.xz * tmp29 * tmp48 + gamma_uu.xx * tmp23 + gamma_uu.xx * tmp21;\
		(resultFlux)->K_ll.zz = gamma_uu.xx * tmp15 + -1. * gamma_uu.xx * tmp16 + gamma_uu.xx * tmp18 + -1. * gamma_uu.xx * tmp19 + gamma_uu.xy * tmp42 + -1. * gamma_uu.xy * tmp44 + -1. * gamma_uu.xy * tmp48 + gamma_uu.xy * tmp46;\
		(resultFlux)->Theta = -1. * Z_l.x * tmp60 + -1. * Z_l.y * tmp61 + -1. * Z_l.z * tmp62 + -1. * dHat_lll.x.yy * tmp64 + -1. * dHat_lll.x.zz * tmp66 + dHat_lll.y.xy * tmp64 + dHat_lll.z.xz * tmp66 + -1. * dDelta_lll.x.yy * tmp64 + -1. * dDelta_lll.x.zz * tmp66 + dDelta_lll.y.xy * tmp64 + dDelta_lll.z.xz * tmp66 + gamma_uu.xx * tmp28 + -1. * gamma_uu.xx * tmp3 + gamma_uu.xx * tmp27 + -1. * gamma_uu.xx * tmp6 + -1. * gamma_uu.xx * tmp8 + -1. * gamma_uu.xx * tmp10 + -1. * gamma_uu.xx * tmp12 + -1. * gamma_uu.xx * tmp14 + gamma_uu.xx * tmp26 + -1. * gamma_uu.xx * tmp17 + gamma_uu.xx * tmp25 + -1. * gamma_uu.xx * tmp20 + gamma_uu.xy * tmp30 + gamma_uu.xy * tmp31 + gamma_uu.xy * tmp32 + gamma_uu.xy * tmp33 + gamma_uu.xy * tmp35 + -1. * gamma_uu.xy * tmp37 + gamma_uu.xy * tmp39 + -1. * gamma_uu.xy * tmp41 + gamma_uu.xy * tmp43 + -1. * gamma_uu.xy * tmp45 + gamma_uu.xy * tmp47 + -1. * gamma_uu.xy * tmp49 + -1. * gamma_uu.xz * tmp52 + gamma_uu.xz * tmp53 + -1. * gamma_uu.xz * tmp54 + gamma_uu.xz * tmp55 + -1. * gamma_uu.xz * tmp56 + gamma_uu.xz * tmp57 + -1. * gamma_uu.xz * tmp58 + gamma_uu.xz * tmp59 + 2. * gamma_uu.xx * tmp22 + 2. * gamma_uu.xx * tmp24 + -2. * gamma_uu.xy * tmp51 + -2. * gamma_uu.xy * tmp50;\
		(resultFlux)->Z_l.x = -1. * Theta * alpha + K_ll.xy * tmp61 + K_ll.xz * tmp62 + K_ll.yy * gamma_uu.yy * alpha + 2. * K_ll.yz * gamma_uu.yz * alpha + K_ll.zz * gamma_uu.zz * alpha;\
		(resultFlux)->Z_l.y = -1. * K_ll.xy * tmp60 + -1. * K_ll.yz * tmp62 + -1. * K_ll.yy * tmp61;\
		(resultFlux)->Z_l.z = -1. * K_ll.xz * tmp60 + -1. * K_ll.zz * tmp62 + -1. * K_ll.yz * tmp61;\
	}\
	<? if false then ?>/*useShift*/\
	{\
		real const tmp1 = 1. / 2.;\
		real const tmp2 = gamma_uu.xx * gamma_uu.xx;\
		real const tmp3 = alpha * alpha;\
		real const tmp4 = tmp2 * tmp3;\
		real const tmp5 = gamma_uu.xy * gamma_uu.xy;\
		real const tmp6 = tmp3 * tmp5;\
		real const tmp7 = gamma_uu.xz * gamma_uu.xz;\
		real const tmp8 = tmp3 * tmp7;\
		real const tmp9 = gamma_uu.xy * tmp3;\
		real const tmp10 = gamma_uu.xz * tmp3;\
		real const tmp11 = dHat_lll.y.xx * tmp3;\
		real const tmp12 = dDelta_lll.y.xx * tmp3;\
		real const tmp13 = dHat_lll.z.xx * tmp3;\
		real const tmp14 = gamma_uu.xz * tmp13;\
		real const tmp15 = dDelta_lll.z.xx * tmp3;\
		real const tmp16 = gamma_uu.xz * tmp15;\
		real const tmp17 = dHat_lll.x.yy * tmp3;\
		real const tmp18 = gamma_uu.yy * tmp17;\
		real const tmp19 = dDelta_lll.x.yy * tmp3;\
		real const tmp20 = gamma_uu.yy * tmp19;\
		real const tmp21 = dHat_lll.x.zz * tmp3;\
		real const tmp22 = gamma_uu.zz * tmp21;\
		real const tmp23 = dDelta_lll.x.zz * tmp3;\
		real const tmp24 = gamma_uu.zz * tmp23;\
		real const tmp25 = dHat_lll.y.yy * tmp3;\
		real const tmp26 = dDelta_lll.y.yy * tmp3;\
		real const tmp27 = dHat_lll.y.zz * tmp3;\
		real const tmp28 = gamma_uu.zz * tmp27;\
		real const tmp29 = dDelta_lll.y.zz * tmp3;\
		real const tmp30 = gamma_uu.zz * tmp29;\
		real const tmp31 = dHat_lll.z.yy * tmp3;\
		real const tmp32 = dDelta_lll.z.yy * tmp3;\
		real const tmp33 = dHat_lll.z.zz * tmp3;\
		real const tmp34 = gamma_uu.zz * tmp33;\
		real const tmp35 = dDelta_lll.z.zz * tmp3;\
		real const tmp36 = gamma_uu.zz * tmp35;\
		real const tmp37 = dHat_lll.x.xy * tmp3;\
		real const tmp38 = dDelta_lll.x.xy * tmp3;\
		real const tmp39 = dHat_lll.x.xz * tmp3;\
		real const tmp40 = dDelta_lll.x.xz * tmp3;\
		real const tmp41 = dHat_lll.y.xy * tmp3;\
		real const tmp42 = gamma_uu.yy * tmp41;\
		real const tmp43 = dDelta_lll.y.xy * tmp3;\
		real const tmp44 = gamma_uu.yy * tmp43;\
		real const tmp45 = dHat_lll.x.yz * tmp3;\
		real const tmp46 = dHat_lll.y.xz * tmp3;\
		real const tmp47 = gamma_uu.yz * tmp46;\
		real const tmp48 = dHat_lll.z.xy * tmp3;\
		real const tmp49 = gamma_uu.yz * tmp48;\
		real const tmp50 = dDelta_lll.x.yz * tmp3;\
		real const tmp51 = dDelta_lll.y.xz * tmp3;\
		real const tmp52 = gamma_uu.yz * tmp51;\
		real const tmp53 = dDelta_lll.z.xy * tmp3;\
		real const tmp54 = gamma_uu.yz * tmp53;\
		real const tmp55 = dHat_lll.z.xz * tmp3;\
		real const tmp56 = gamma_uu.zz * tmp55;\
		real const tmp57 = dDelta_lll.z.xz * tmp3;\
		real const tmp58 = gamma_uu.zz * tmp57;\
		real const tmp59 = gamma_uu.yz * tmp31;\
		real const tmp60 = gamma_uu.yz * tmp32;\
		real const tmp61 = dHat_lll.z.yz * tmp3;\
		real const tmp62 = gamma_uu.zz * tmp61;\
		real const tmp63 = dDelta_lll.z.yz * tmp3;\
		real const tmp64 = gamma_uu.zz * tmp63;\
		real const tmp65 = dHat_lll.y.yz * tmp3;\
		real const tmp66 = dDelta_lll.y.yz * tmp3;\
		real const tmp67 = gamma_uu.yy * gamma_uu.yy;\
		real const tmp68 = tmp3 * tmp67;\
		real const tmp69 = gamma_uu.yz * gamma_uu.yz;\
		real const tmp70 = tmp3 * tmp69;\
		real const tmp71 = gamma_uu.yz * tmp3;\
		real const tmp72 = dHat_lll.x.xx * tmp3;\
		real const tmp73 = dDelta_lll.x.xx * tmp3;\
		real const tmp74 = gamma_uu.zz * gamma_uu.zz;\
		real const tmp75 = tmp3 * tmp74;\
		(resultFlux)->a_l.x += -1. * beta_u.x * a_l.x;\
		(resultFlux)->a_l.y += -1. * beta_u.x * a_l.y;\
		(resultFlux)->a_l.z += -1. * beta_u.x * a_l.z;\
		...
		(resultFlux)->d_lll.x.xy += -1. * gammaHat_ll.xx * b_ul.x.y * tmp1 + -1. * gammaHat_ll.xy * b_ul.x.x * tmp1 + -1. * gammaHat_ll.xy * b_ul.y.y * tmp1 + -1. * gammaHat_ll.xz * b_ul.z.y * tmp1 + -1. * gammaHat_ll.yy * b_ul.y.x * tmp1 + -1. * gammaHat_ll.yz * b_ul.z.x * tmp1 + -1. * gammaDelta_ll.xx * b_ul.x.y * tmp1 + -1. * gammaDelta_ll.xy * b_ul.x.x * tmp1 + -1. * gammaDelta_ll.xy * b_ul.y.y * tmp1 + -1. * gammaDelta_ll.xz * b_ul.z.y * tmp1 + -1. * gammaDelta_ll.yy * b_ul.y.x * tmp1 + -1. * gammaDelta_ll.yz * b_ul.z.x * tmp1 + -1. * beta_u.x * dHat_lll.x.xy + -1. * beta_u.x * dDelta_lll.x.xy + -1. * beta_u.z * dHat_lll.z.xy + -1. * beta_u.y * dHat_lll.y.xy;\
		(resultFlux)->d_lll.x.xz += -1. * gammaHat_ll.xx * b_ul.x.z * tmp1 + -1. * gammaHat_ll.xy * b_ul.y.z * tmp1 + -1. * gammaHat_ll.xz * b_ul.x.x * tmp1 + -1. * gammaHat_ll.xz * b_ul.z.z * tmp1 + -1. * gammaHat_ll.yz * b_ul.y.x * tmp1 + -1. * gammaHat_ll.zz * b_ul.z.x * tmp1 + -1. * gammaDelta_ll.xx * b_ul.x.z * tmp1 + -1. * gammaDelta_ll.xy * b_ul.y.z * tmp1 + -1. * gammaDelta_ll.xz * b_ul.x.x * tmp1 + -1. * gammaDelta_ll.xz * b_ul.z.z * tmp1 + -1. * gammaDelta_ll.yz * b_ul.y.x * tmp1 + -1. * gammaDelta_ll.zz * b_ul.z.x * tmp1 + -1. * beta_u.x * dHat_lll.x.xz + -1. * beta_u.x * dDelta_lll.x.xz + -1. * beta_u.z * dHat_lll.z.xz + -1. * beta_u.y * dHat_lll.y.xz;\
		(resultFlux)->d_lll.x.yy += -1. * beta_u.x * dHat_lll.x.yy + -1. * beta_u.x * dDelta_lll.x.yy + -1. * beta_u.y * dHat_lll.y.yy + -1. * beta_u.z * dHat_lll.z.yy + -1. * gammaHat_ll.xy * b_ul.x.y + -1. * gammaHat_ll.yy * b_ul.y.y + -1. * gammaHat_ll.yz * b_ul.z.y + -1. * gammaDelta_ll.xy * b_ul.x.y + -1. * gammaDelta_ll.yz * b_ul.z.y + -1. * gammaDelta_ll.yy * b_ul.y.y;\
		(resultFlux)->d_lll.x.yz += -1. * gammaHat_ll.xy * b_ul.x.z * tmp1 + -1. * gammaHat_ll.xz * b_ul.x.y * tmp1 + -1. * gammaHat_ll.yy * b_ul.y.z * tmp1 + -1. * gammaHat_ll.yz * b_ul.y.y * tmp1 + -1. * gammaHat_ll.yz * b_ul.z.z * tmp1 + -1. * gammaHat_ll.zz * b_ul.z.y * tmp1 + -1. * gammaDelta_ll.xy * b_ul.x.z * tmp1 + -1. * gammaDelta_ll.xz * b_ul.x.y * tmp1 + -1. * gammaDelta_ll.yy * b_ul.y.z * tmp1 + -1. * gammaDelta_ll.yz * b_ul.y.y * tmp1 + -1. * gammaDelta_ll.yz * b_ul.z.z * tmp1 + -1. * gammaDelta_ll.zz * b_ul.z.y * tmp1 + -1. * beta_u.x * dHat_lll.x.yz + -1. * beta_u.x * dDelta_lll.x.yz + -1. * beta_u.z * dHat_lll.z.yz + -1. * beta_u.y * dHat_lll.y.yz;\
		(resultFlux)->d_lll.x.zz += -1. * beta_u.x * dHat_lll.x.zz + -1. * beta_u.x * dDelta_lll.x.zz + -1. * beta_u.y * dHat_lll.y.zz + -1. * beta_u.z * dHat_lll.z.zz + -1. * gammaHat_ll.xz * b_ul.x.z + -1. * gammaHat_ll.yz * b_ul.y.z + -1. * gammaHat_ll.zz * b_ul.z.z + -1. * gammaDelta_ll.xz * b_ul.x.z + -1. * gammaDelta_ll.zz * b_ul.z.z + -1. * gammaDelta_ll.yz * b_ul.y.z;\
		(resultFlux)->d_lll.y.xx += -1. * beta_u.x * dDelta_lll.y.xx;\
		(resultFlux)->d_lll.y.xy += -1. * beta_u.x * dDelta_lll.y.xy;\
		(resultFlux)->d_lll.y.xz += -1. * beta_u.x * dDelta_lll.y.xz;\
		(resultFlux)->d_lll.y.yy += -1. * beta_u.x * dDelta_lll.y.yy;\
		(resultFlux)->d_lll.y.yz += -1. * beta_u.x * dDelta_lll.y.yz;\
		(resultFlux)->d_lll.y.zz += -1. * beta_u.x * dDelta_lll.y.zz;\
		(resultFlux)->d_lll.z.xx += -1. * beta_u.x * dDelta_lll.z.xx;\
		(resultFlux)->d_lll.z.xy += -1. * beta_u.x * dDelta_lll.z.xy;\
		(resultFlux)->d_lll.z.xz += -1. * beta_u.x * dDelta_lll.z.xz;\
		(resultFlux)->d_lll.z.yy += -1. * beta_u.x * dDelta_lll.z.yy;\
		(resultFlux)->d_lll.z.yz += -1. * beta_u.x * dDelta_lll.z.yz;\
		(resultFlux)->d_lll.z.zz += -1. * beta_u.x * dDelta_lll.z.zz;\
		...
		(resultFlux)->b_ul.x.x = -1. * beta_u.x * b_ul.x.x + -1. * beta_u.y * b_ul.x.y + -1. * beta_u.z * b_ul.x.z + -1. * dHat_lll.x.xx * tmp4 + -1. * dDelta_lll.x.xx * tmp4 + -2. * dHat_lll.x.yy * tmp6 + -2. * dHat_lll.x.zz * tmp8 + -2. * dDelta_lll.x.yy * tmp6 + -2. * dDelta_lll.x.zz * tmp8 + a_l.x * gamma_uu.xx * tmp3 + a_l.y * tmp9 + a_l.z * tmp10 + -1. * gamma_uu.xx * gamma_uu.xy * tmp11 + -1. * gamma_uu.xx * gamma_uu.xy * tmp12 + -1. * gamma_uu.xx * tmp14 + -1. * gamma_uu.xx * tmp16 + gamma_uu.xx * tmp18 + gamma_uu.xx * tmp20 + gamma_uu.xx * tmp22 + gamma_uu.xx * tmp24 + -1. * gamma_uu.xy * gamma_uu.yy * tmp25 + -1. * gamma_uu.xy * gamma_uu.yy * tmp26 + gamma_uu.xy * tmp28 + gamma_uu.xy * tmp30 + gamma_uu.xz * gamma_uu.yy * tmp31 + gamma_uu.xz * gamma_uu.yy * tmp32 + -1. * gamma_uu.xz * tmp34 + -1. * gamma_uu.xz * tmp36 + -2. * gamma_uu.xx * gamma_uu.xy * tmp37 + -2. * gamma_uu.xx * gamma_uu.xy * tmp38 + -2. * gamma_uu.xx * gamma_uu.xz * tmp39 + -2. * gamma_uu.xx * gamma_uu.xz * tmp40 + -2. * gamma_uu.xx * tmp42 + -2. * gamma_uu.xx * tmp44 + 2. * gamma_uu.xx * gamma_uu.yz * tmp45 + -2. * gamma_uu.xx * tmp47 + -2. * gamma_uu.xx * tmp49 + 2. * gamma_uu.xx * gamma_uu.yz * tmp50 + -2. * gamma_uu.xx * tmp52 + -2. * gamma_uu.xx * tmp54 + -2. * gamma_uu.xx * tmp56 + -2. * gamma_uu.xx * tmp58 + -2. * gamma_uu.xy * tmp59 + -2. * gamma_uu.xy * tmp60 + -2. * gamma_uu.xy * tmp62 + -2. * gamma_uu.xy * tmp64 + -2. * gamma_uu.xz * gamma_uu.yy * tmp65 + -2. * gamma_uu.xz * gamma_uu.yy * tmp66 + -2. * gamma_uu.xz * gamma_uu.yz * tmp27 + -2. * gamma_uu.xz * gamma_uu.yz * tmp29 + -4. * gamma_uu.xy * gamma_uu.xz * tmp50 + -4. * gamma_uu.xy * gamma_uu.xz * tmp45;\
		(resultFlux)->b_ul.x.y = 0.;\
		(resultFlux)->b_ul.x.z = 0.;\
		(resultFlux)->b_ul.y.x = -1. * beta_u.x * b_ul.y.x + -1. * beta_u.y * b_ul.y.y + -1. * beta_u.z * b_ul.y.z + -1. * dHat_lll.y.yy * tmp68 + -1. * dDelta_lll.y.yy * tmp68 + -2. * dHat_lll.y.xx * tmp6 + -2. * dHat_lll.y.zz * tmp70 + -2. * dDelta_lll.y.xx * tmp6 + -2. * dDelta_lll.y.zz * tmp70 + a_l.x * tmp9 + a_l.y * gamma_uu.yy * tmp3 + a_l.z * tmp71 + -1. * gamma_uu.xx * gamma_uu.xy * tmp72 + -1. * gamma_uu.xx * gamma_uu.xy * tmp73 + gamma_uu.xx * gamma_uu.yy * tmp11 + gamma_uu.xx * gamma_uu.yy * tmp12 + gamma_uu.xx * gamma_uu.yz * tmp13 + gamma_uu.xx * gamma_uu.yz * tmp15 + -1. * gamma_uu.xy * tmp18 + -1. * gamma_uu.xy * tmp20 + gamma_uu.xy * tmp22 + gamma_uu.xy * tmp24 + -1. * gamma_uu.yy * tmp59 + -1. * gamma_uu.yy * tmp60 + gamma_uu.yy * tmp28 + gamma_uu.yy * tmp30 + -1. * gamma_uu.yz * tmp34 + -1. * gamma_uu.yz * tmp36 + -2. * gamma_uu.xx * gamma_uu.yy * tmp37 + -2. * gamma_uu.xx * gamma_uu.yy * tmp38 + -2. * gamma_uu.xx * gamma_uu.yz * tmp39 + -2. * gamma_uu.xx * gamma_uu.yz * tmp40 + -2. * gamma_uu.xy * tmp14 + -2. * gamma_uu.xy * tmp16 + -2. * gamma_uu.xy * tmp42 + -2. * gamma_uu.xy * tmp44 + -2. * gamma_uu.xy * tmp56 + -2. * gamma_uu.xy * tmp58 + -2. * gamma_uu.xz * gamma_uu.yy * tmp45 + 2. * gamma_uu.xz * gamma_uu.yy * tmp46 + -2. * gamma_uu.xz * gamma_uu.yy * tmp48 + -2. * gamma_uu.xz * gamma_uu.yy * tmp50 + 2. * gamma_uu.xz * gamma_uu.yy * tmp51 + -2. * gamma_uu.xz * gamma_uu.yy * tmp53 + -2. * gamma_uu.xz * gamma_uu.yz * tmp21 + -2. * gamma_uu.xz * gamma_uu.yz * tmp23 + -2. * gamma_uu.yy * gamma_uu.yz * tmp65 + -2. * gamma_uu.yy * gamma_uu.yz * tmp66 + -2. * gamma_uu.yy * tmp62 + -2. * gamma_uu.yy * tmp64 + -4. * gamma_uu.xy * tmp52 + -4. * gamma_uu.xy * tmp47;\
		(resultFlux)->b_ul.y.y = 0.;\
		(resultFlux)->b_ul.y.z = 0.;\
		(resultFlux)->b_ul.z.x = -1. * beta_u.x * b_ul.z.x + -1. * beta_u.y * b_ul.z.y + -1. * beta_u.z * b_ul.z.z + -1. * dHat_lll.z.zz * tmp75 + -1. * dDelta_lll.z.zz * tmp75 + -2. * dHat_lll.z.xx * tmp8 + -2. * dHat_lll.z.yy * tmp70 + -2. * dDelta_lll.z.xx * tmp8 + -2. * dDelta_lll.z.yy * tmp70 + a_l.x * tmp10 + a_l.y * tmp71 + a_l.z * gamma_uu.zz * tmp3 + -1. * gamma_uu.xx * gamma_uu.xz * tmp72 + -1. * gamma_uu.xx * gamma_uu.xz * tmp73 + gamma_uu.xx * gamma_uu.yz * tmp11 + gamma_uu.xx * gamma_uu.yz * tmp12 + gamma_uu.xx * gamma_uu.zz * tmp13 + gamma_uu.xx * gamma_uu.zz * tmp15 + gamma_uu.xz * tmp18 + gamma_uu.xz * tmp20 + -1. * gamma_uu.xz * tmp22 + -1. * gamma_uu.xz * tmp24 + -1. * gamma_uu.yy * gamma_uu.yz * tmp25 + -1. * gamma_uu.yy * gamma_uu.yz * tmp26 + gamma_uu.yy * gamma_uu.zz * tmp31 + gamma_uu.yy * gamma_uu.zz * tmp32 + -1. * gamma_uu.yz * tmp28 + -1. * gamma_uu.yz * tmp30 + -2. * gamma_uu.xx * gamma_uu.yz * tmp37 + -2. * gamma_uu.xx * gamma_uu.yz * tmp38 + -2. * gamma_uu.xx * gamma_uu.zz * tmp39 + -2. * gamma_uu.xx * gamma_uu.zz * tmp40 + -2. * gamma_uu.xy * gamma_uu.xz * tmp11 + -2. * gamma_uu.xy * gamma_uu.xz * tmp12 + -2. * gamma_uu.xy * gamma_uu.yz * tmp17 + -2. * gamma_uu.xy * gamma_uu.yz * tmp19 + -2. * gamma_uu.xy * gamma_uu.zz * tmp45 + -2. * gamma_uu.xy * gamma_uu.zz * tmp46 + 2. * gamma_uu.xy * gamma_uu.zz * tmp48 + -2. * gamma_uu.xy * gamma_uu.zz * tmp50 + -2. * gamma_uu.xy * gamma_uu.zz * tmp51 + 2. * gamma_uu.xy * gamma_uu.zz * tmp53 + -2. * gamma_uu.xz * tmp42 + -2. * gamma_uu.xz * tmp44 + -2. * gamma_uu.xz * tmp56 + -2. * gamma_uu.xz * tmp58 + -2. * gamma_uu.yy * gamma_uu.zz * tmp65 + -2. * gamma_uu.yy * gamma_uu.zz * tmp66 + -2. * gamma_uu.yz * tmp62 + -2. * gamma_uu.yz * tmp64 + -4. * gamma_uu.xz * tmp54 + -4. * gamma_uu.xz * tmp49;\
