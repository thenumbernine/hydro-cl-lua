...
<? if true then ?>\
	...
	{\
		real const tmp1 = K_ll.xy * alpha;\
		real const tmp2 = K_ll.xz * alpha;\
		real const tmp3 = Z_l.x * alpha;\
		real const tmp4 = dHat_lll.x.yy * alpha;\
		real const tmp5 = dHat_lll.y.xy * alpha;\
		real const tmp6 = gamma_uu.yy * tmp5;\
		real const tmp7 = dDelta_lll.x.yy * alpha;\
		real const tmp8 = dDelta_lll.y.xy * alpha;\
		real const tmp9 = gamma_uu.yy * tmp8;\
		real const tmp10 = dHat_lll.y.xz * alpha;\
		real const tmp11 = gamma_uu.yz * tmp10;\
		real const tmp12 = dHat_lll.z.xy * alpha;\
		real const tmp13 = gamma_uu.yz * tmp12;\
		real const tmp14 = dDelta_lll.y.xz * alpha;\
		real const tmp15 = gamma_uu.yz * tmp14;\
		real const tmp16 = dDelta_lll.z.xy * alpha;\
		real const tmp17 = gamma_uu.yz * tmp16;\
		real const tmp18 = dHat_lll.x.zz * alpha;\
		real const tmp19 = dHat_lll.z.xz * alpha;\
		real const tmp20 = gamma_uu.zz * tmp19;\
		real const tmp21 = dDelta_lll.x.zz * alpha;\
		real const tmp22 = dDelta_lll.z.xz * alpha;\
		real const tmp23 = gamma_uu.zz * tmp22;\
		real const tmp24 = dHat_lll.x.yz * alpha;\
		real const tmp25 = gamma_uu.yz * tmp24;\
		real const tmp26 = dDelta_lll.x.yz * alpha;\
		real const tmp27 = gamma_uu.yz * tmp26;\
		real const tmp28 = gamma_uu.zz * tmp21;\
		real const tmp29 = gamma_uu.zz * tmp18;\
		real const tmp30 = -1. * tmp17;\
		real const tmp31 = -1. * tmp15;\
		real const tmp32 = -1. * tmp13;\
		real const tmp33 = -1. * tmp11;\
		real const tmp34 = gamma_uu.yy * tmp7;\
		real const tmp35 = gamma_uu.yy * tmp4;\
		real const tmp36 = a_l.x * alpha;\
		real const tmp37 = dHat_lll.x.xy * alpha;\
		real const tmp38 = dHat_lll.y.xx * alpha;\
		real const tmp39 = dDelta_lll.x.xy * alpha;\
		real const tmp40 = dDelta_lll.y.xx * alpha;\
		real const tmp41 = dHat_lll.x.xz * alpha;\
		real const tmp42 = dHat_lll.z.xx * alpha;\
		real const tmp43 = dDelta_lll.x.xz * alpha;\
		real const tmp44 = dDelta_lll.z.xx * alpha;\
		real const tmp45 = 1. / 2.;\
		real const tmp46 = gamma_uu.xz * tmp41;\
		real const tmp47 = gamma_uu.xz * tmp42;\
		real const tmp48 = gamma_uu.xz * tmp43;\
		real const tmp49 = gamma_uu.xz * tmp44;\
		real const tmp50 = gamma_uu.xy * tmp38;\
		real const tmp51 = gamma_uu.xy * tmp40;\
		real const tmp52 = gamma_uu.xy * tmp39;\
		real const tmp53 = gamma_uu.xy * tmp37;\
		real const tmp54 = tmp27 * tmp45;\
		real const tmp55 = tmp25 * tmp45;\
		real const tmp56 = -1. * tmp3;\
		real const tmp57 = tmp36 * tmp45;\
		real const tmp58 = gamma_uu.xx * alpha;\
		real const tmp59 = gamma_uu.xy * alpha;\
		real const tmp60 = gamma_uu.xz * alpha;\
		real const tmp61 = gamma_uu.xy * gamma_uu.xy;\
		real const tmp62 = alpha * tmp61;\
		real const tmp63 = gamma_uu.xz * gamma_uu.xz;\
		real const tmp64 = alpha * tmp63;\
		real const tmp65 = dHat_lll.y.yz * alpha;\
		real const tmp66 = dHat_lll.z.yy * alpha;\
		real const tmp67 = dDelta_lll.y.yz * alpha;\
		real const tmp68 = dDelta_lll.z.yy * alpha;\
		real const tmp69 = dHat_lll.y.zz * alpha;\
		real const tmp70 = dHat_lll.z.yz * alpha;\
		real const tmp71 = dDelta_lll.y.zz * alpha;\
		real const tmp72 = dDelta_lll.z.yz * alpha;\
		real const tmp73 = dHat_t_ll.xy + tmp1;\
		real const tmp74 = dHat_t_ll.xz + tmp2;\
		(resultFlux)->a_l.x = -2. * f_alpha * Theta + K_ll.xx * gamma_uu.xx * f_alpha + K_ll.yy * gamma_uu.yy * f_alpha + K_ll.zz * gamma_uu.zz * f_alpha + 2. * K_ll.xy * gamma_uu.xy * f_alpha + 2. * K_ll.yz * gamma_uu.yz * f_alpha + 2. * K_ll.xz * gamma_uu.xz * f_alpha;\
		(resultFlux)->a_l.y = 0.;\
		(resultFlux)->a_l.z = 0.;\
		(resultFlux)->d_lll.x.xx = dHat_t_ll.xx + K_ll.xx * alpha;\
		(resultFlux)->d_lll.x.xy = tmp73;\
		(resultFlux)->d_lll.x.xz = tmp74;\
		(resultFlux)->d_lll.x.yy = 0.;\
		(resultFlux)->d_lll.x.yz = 0.;\
		(resultFlux)->d_lll.x.zz = 0.;\
		(resultFlux)->d_lll.y.xx = 0.;\
		(resultFlux)->d_lll.y.xy = 0.;\
		(resultFlux)->d_lll.y.xz = 0.;\
		(resultFlux)->d_lll.y.yy = tmp73;\
		(resultFlux)->d_lll.y.yz = tmp74;\
		(resultFlux)->d_lll.y.zz = 0.;\
		(resultFlux)->d_lll.z.xx = 0.;\
		(resultFlux)->d_lll.z.xy = 0.;\
		(resultFlux)->d_lll.z.xz = 0.;\
		(resultFlux)->d_lll.z.yy = 0.;\
		(resultFlux)->d_lll.z.yz = 0.;\
		(resultFlux)->d_lll.z.zz = tmp74;\
		(resultFlux)->K_ll.xx = -2. * tmp3 + -1. * tmp6 + -1. * tmp9 + -1. * tmp20 + -1. * tmp23 + 2. * tmp27 + 2. * tmp25 + tmp28 + tmp29 + tmp30 + tmp31 + tmp32 + tmp33 + tmp34 + tmp35 + tmp36;\
		(resultFlux)->K_ll.xy = -1. * gamma_uu.yy * tmp37 + gamma_uu.yy * tmp38 + -1. * gamma_uu.yy * tmp39 + gamma_uu.yy * tmp40 + -1. * gamma_uu.yz * tmp41 + gamma_uu.yz * tmp42 + gamma_uu.yz * tmp44 + -1. * gamma_uu.yz * tmp43;\
		(resultFlux)->K_ll.xz = -1. * gamma_uu.yz * tmp37 + gamma_uu.yz * tmp38 + -1. * gamma_uu.yz * tmp39 + gamma_uu.yz * tmp40 + -1. * gamma_uu.zz * tmp41 + gamma_uu.zz * tmp42 + gamma_uu.zz * tmp44 + -1. * gamma_uu.zz * tmp43;\
		(resultFlux)->K_ll.yy = tmp45 * tmp46 + -1. * tmp45 * tmp47 + tmp45 * tmp48 + -1. * tmp45 * tmp49 + tmp13 * tmp45 + tmp17 * tmp45 + tmp29 * tmp45 + -1. * tmp20 * tmp45 + tmp28 * tmp45 + -1. * tmp23 * tmp45 + -1. * tmp50 + -1. * tmp51 + tmp31 + tmp33 + tmp52 + tmp53 + tmp54 + tmp55 + tmp56 + tmp57;\
		(resultFlux)->K_ll.yz = gamma_uu.xz * tmp37 * tmp45 + -1. * gamma_uu.xz * tmp38 * tmp45 + gamma_uu.xz * tmp39 * tmp45 + -1. * gamma_uu.xz * tmp40 * tmp45 + -1. * gamma_uu.yz * tmp4 * tmp45 + gamma_uu.yz * tmp5 * tmp45 + -1. * gamma_uu.yz * tmp7 * tmp45 + gamma_uu.yz * tmp8 * tmp45 + -1. * gamma_uu.zz * tmp24 * tmp45 + -1. * gamma_uu.zz * tmp10 * tmp45 + -1. * gamma_uu.zz * tmp26 * tmp45 + -1. * gamma_uu.zz * tmp14 * tmp45 + gamma_uu.zz * tmp16 + gamma_uu.zz * tmp12;\
		(resultFlux)->K_ll.zz = tmp45 * tmp53 + -1. * tmp45 * tmp50 + tmp45 * tmp52 + -1. * tmp45 * tmp51 + tmp35 * tmp45 + -1. * tmp6 * tmp45 + tmp34 * tmp45 + -1. * tmp9 * tmp45 + tmp11 * tmp45 + tmp15 * tmp45 + -1. * tmp47 + -1. * tmp49 + tmp30 + tmp32 + tmp46 + tmp48 + tmp54 + tmp55 + tmp56 + tmp57;\
		(resultFlux)->Theta = -1. * Z_l.x * tmp58 + -1. * Z_l.y * tmp59 + -1. * Z_l.z * tmp60 + -1. * dHat_lll.x.yy * tmp62 + -1. * dHat_lll.x.zz * tmp64 + dHat_lll.y.xy * tmp62 + dHat_lll.z.xz * tmp64 + -1. * dDelta_lll.x.yy * tmp62 + -1. * dDelta_lll.x.zz * tmp64 + dDelta_lll.y.xy * tmp62 + dDelta_lll.z.xz * tmp64 + gamma_uu.xx * tmp35 + -1. * gamma_uu.xx * tmp6 + gamma_uu.xx * tmp34 + -1. * gamma_uu.xx * tmp9 + -1. * gamma_uu.xx * tmp11 + -1. * gamma_uu.xx * tmp13 + -1. * gamma_uu.xx * tmp15 + -1. * gamma_uu.xx * tmp17 + gamma_uu.xx * tmp29 + -1. * gamma_uu.xx * tmp20 + gamma_uu.xx * tmp28 + -1. * gamma_uu.xx * tmp23 + gamma_uu.xy * gamma_uu.xz * tmp10 + gamma_uu.xy * gamma_uu.xz * tmp12 + gamma_uu.xy * gamma_uu.xz * tmp14 + gamma_uu.xy * gamma_uu.xz * tmp16 + gamma_uu.xy * gamma_uu.yz * tmp65 + -1. * gamma_uu.xy * gamma_uu.yz * tmp66 + gamma_uu.xy * gamma_uu.yz * tmp67 + -1. * gamma_uu.xy * gamma_uu.yz * tmp68 + gamma_uu.xy * gamma_uu.zz * tmp69 + -1. * gamma_uu.xy * gamma_uu.zz * tmp70 + gamma_uu.xy * gamma_uu.zz * tmp71 + -1. * gamma_uu.xy * gamma_uu.zz * tmp72 + -1. * gamma_uu.xz * gamma_uu.yy * tmp65 + gamma_uu.xz * gamma_uu.yy * tmp66 + -1. * gamma_uu.xz * gamma_uu.yy * tmp67 + gamma_uu.xz * gamma_uu.yy * tmp68 + -1. * gamma_uu.xz * gamma_uu.yz * tmp69 + gamma_uu.xz * gamma_uu.yz * tmp70 + -1. * gamma_uu.xz * gamma_uu.yz * tmp71 + gamma_uu.xz * gamma_uu.yz * tmp72 + 2. * gamma_uu.xx * tmp25 + 2. * gamma_uu.xx * tmp27 + -2. * gamma_uu.xy * gamma_uu.xz * tmp26 + -2. * gamma_uu.xy * gamma_uu.xz * tmp24;\
		(resultFlux)->Z_l.x = -1. * Theta * alpha + K_ll.xy * tmp59 + K_ll.xz * tmp60 + K_ll.yy * gamma_uu.yy * alpha + 2. * K_ll.yz * gamma_uu.yz * alpha + K_ll.zz * gamma_uu.zz * alpha;\
		(resultFlux)->Z_l.y = -1. * K_ll.xy * tmp58 + -1. * K_ll.yz * tmp60 + -1. * K_ll.yy * tmp59;\
		(resultFlux)->Z_l.z = -1. * K_ll.xz * tmp58 + -1. * K_ll.zz * tmp60 + -1. * K_ll.yz * tmp59;\
	}\
	<? if false then ?>/*useShift*/\
	{\
		real const tmp1 = 1. / 2.;\
		real const tmp2 = gammaHat_ll.xx * b_ul.x.y;\
		real const tmp3 = tmp1 * tmp2;\
		real const tmp4 = gammaHat_ll.xy * b_ul.x.x;\
		real const tmp5 = tmp1 * tmp4;\
		real const tmp6 = gammaHat_ll.xy * b_ul.y.y;\
		real const tmp7 = tmp1 * tmp6;\
		real const tmp8 = gammaHat_ll.xz * b_ul.z.y;\
		real const tmp9 = tmp1 * tmp8;\
		real const tmp10 = gammaHat_ll.yy * b_ul.y.x;\
		real const tmp11 = tmp1 * tmp10;\
		real const tmp12 = gammaHat_ll.yz * b_ul.z.x;\
		real const tmp13 = tmp1 * tmp12;\
		real const tmp14 = gammaDelta_ll.xx * b_ul.x.y;\
		real const tmp15 = tmp1 * tmp14;\
		real const tmp16 = gammaDelta_ll.xy * b_ul.x.x;\
		real const tmp17 = tmp1 * tmp16;\
		real const tmp18 = gammaDelta_ll.xy * b_ul.y.y;\
		real const tmp19 = tmp1 * tmp18;\
		real const tmp20 = gammaDelta_ll.xz * b_ul.z.y;\
		real const tmp21 = tmp1 * tmp20;\
		real const tmp22 = gammaDelta_ll.yy * b_ul.y.x;\
		real const tmp23 = tmp1 * tmp22;\
		real const tmp24 = gammaDelta_ll.yz * b_ul.z.x;\
		real const tmp25 = tmp1 * tmp24;\
		real const tmp26 = beta_u.x * dHat_lll.x.xy;\
		real const tmp27 = beta_u.y * dHat_lll.y.xy;\
		real const tmp28 = beta_u.z * dHat_lll.z.xy;\
		real const tmp29 = -1. * tmp27;\
		real const tmp30 = -1. * tmp28;\
		real const tmp31 = -1. * tmp26;\
		real const tmp32 = -1. * tmp25;\
		real const tmp33 = -1. * tmp23;\
		real const tmp34 = -1. * tmp21;\
		real const tmp35 = -1. * tmp19;\
		real const tmp36 = -1. * tmp17;\
		real const tmp37 = -1. * tmp15;\
		real const tmp38 = -1. * tmp13;\
		real const tmp39 = -1. * tmp11;\
		real const tmp40 = -1. * tmp9;\
		real const tmp41 = -1. * tmp7;\
		real const tmp42 = -1. * tmp5;\
		real const tmp43 = -1. * tmp3;\
		real const tmp44 = gammaHat_ll.xx * b_ul.x.z;\
		real const tmp45 = tmp1 * tmp44;\
		real const tmp46 = gammaHat_ll.xy * b_ul.y.z;\
		real const tmp47 = tmp1 * tmp46;\
		real const tmp48 = gammaHat_ll.xz * b_ul.x.x;\
		real const tmp49 = tmp1 * tmp48;\
		real const tmp50 = gammaHat_ll.xz * b_ul.z.z;\
		real const tmp51 = tmp1 * tmp50;\
		real const tmp52 = gammaHat_ll.yz * b_ul.y.x;\
		real const tmp53 = tmp1 * tmp52;\
		real const tmp54 = gammaHat_ll.zz * b_ul.z.x;\
		real const tmp55 = tmp1 * tmp54;\
		real const tmp56 = gammaDelta_ll.xx * b_ul.x.z;\
		real const tmp57 = tmp1 * tmp56;\
		real const tmp58 = gammaDelta_ll.xy * b_ul.y.z;\
		real const tmp59 = tmp1 * tmp58;\
		real const tmp60 = gammaDelta_ll.xz * b_ul.x.x;\
		real const tmp61 = tmp1 * tmp60;\
		real const tmp62 = gammaDelta_ll.xz * b_ul.z.z;\
		real const tmp63 = tmp1 * tmp62;\
		real const tmp64 = gammaDelta_ll.yz * b_ul.y.x;\
		real const tmp65 = tmp1 * tmp64;\
		real const tmp66 = gammaDelta_ll.zz * b_ul.z.x;\
		real const tmp67 = tmp1 * tmp66;\
		real const tmp68 = beta_u.x * dHat_lll.x.xz;\
		real const tmp69 = beta_u.y * dHat_lll.y.xz;\
		real const tmp70 = beta_u.z * dHat_lll.z.xz;\
		real const tmp71 = -1. * tmp69;\
		real const tmp72 = -1. * tmp70;\
		real const tmp73 = -1. * tmp68;\
		real const tmp74 = -1. * tmp67;\
		real const tmp75 = -1. * tmp65;\
		real const tmp76 = -1. * tmp63;\
		real const tmp77 = -1. * tmp61;\
		real const tmp78 = -1. * tmp59;\
		real const tmp79 = -1. * tmp57;\
		real const tmp80 = -1. * tmp55;\
		real const tmp81 = -1. * tmp53;\
		real const tmp82 = -1. * tmp51;\
		real const tmp83 = -1. * tmp49;\
		real const tmp84 = -1. * tmp47;\
		real const tmp85 = -1. * tmp45;\
		real const tmp86 = gamma_uu.xx * gamma_uu.xx;\
		real const tmp87 = alpha * alpha;\
		real const tmp88 = tmp86 * tmp87;\
		real const tmp89 = gamma_uu.xy * gamma_uu.xy;\
		real const tmp90 = tmp87 * tmp89;\
		real const tmp91 = gamma_uu.xz * gamma_uu.xz;\
		real const tmp92 = tmp87 * tmp91;\
		real const tmp93 = gamma_uu.xy * tmp87;\
		real const tmp94 = gamma_uu.xz * tmp87;\
		real const tmp95 = dHat_lll.y.xx * tmp87;\
		real const tmp96 = dDelta_lll.y.xx * tmp87;\
		real const tmp97 = dHat_lll.z.xx * tmp87;\
		real const tmp98 = gamma_uu.xz * tmp97;\
		real const tmp99 = dDelta_lll.z.xx * tmp87;\
		real const tmp100 = gamma_uu.xz * tmp99;\
		real const tmp101 = dHat_lll.x.yy * tmp87;\
		real const tmp102 = gamma_uu.yy * tmp101;\
		real const tmp103 = dDelta_lll.x.yy * tmp87;\
		real const tmp104 = gamma_uu.yy * tmp103;\
		real const tmp105 = dHat_lll.x.zz * tmp87;\
		real const tmp106 = gamma_uu.zz * tmp105;\
		real const tmp107 = dDelta_lll.x.zz * tmp87;\
		real const tmp108 = gamma_uu.zz * tmp107;\
		real const tmp109 = dHat_lll.y.yy * tmp87;\
		real const tmp110 = dDelta_lll.y.yy * tmp87;\
		real const tmp111 = dHat_lll.y.zz * tmp87;\
		real const tmp112 = gamma_uu.zz * tmp111;\
		real const tmp113 = dDelta_lll.y.zz * tmp87;\
		real const tmp114 = gamma_uu.zz * tmp113;\
		real const tmp115 = dHat_lll.z.yy * tmp87;\
		real const tmp116 = dDelta_lll.z.yy * tmp87;\
		real const tmp117 = dHat_lll.z.zz * tmp87;\
		real const tmp118 = gamma_uu.zz * tmp117;\
		real const tmp119 = dDelta_lll.z.zz * tmp87;\
		real const tmp120 = gamma_uu.zz * tmp119;\
		real const tmp121 = dHat_lll.x.xy * tmp87;\
		real const tmp122 = dDelta_lll.x.xy * tmp87;\
		real const tmp123 = dHat_lll.x.xz * tmp87;\
		real const tmp124 = dDelta_lll.x.xz * tmp87;\
		real const tmp125 = dHat_lll.y.xy * tmp87;\
		real const tmp126 = gamma_uu.yy * tmp125;\
		real const tmp127 = dDelta_lll.y.xy * tmp87;\
		real const tmp128 = gamma_uu.yy * tmp127;\
		real const tmp129 = dHat_lll.x.yz * tmp87;\
		real const tmp130 = dHat_lll.y.xz * tmp87;\
		real const tmp131 = gamma_uu.yz * tmp130;\
		real const tmp132 = dHat_lll.z.xy * tmp87;\
		real const tmp133 = gamma_uu.yz * tmp132;\
		real const tmp134 = dDelta_lll.x.yz * tmp87;\
		real const tmp135 = dDelta_lll.y.xz * tmp87;\
		real const tmp136 = gamma_uu.yz * tmp135;\
		real const tmp137 = dDelta_lll.z.xy * tmp87;\
		real const tmp138 = gamma_uu.yz * tmp137;\
		real const tmp139 = dHat_lll.z.xz * tmp87;\
		real const tmp140 = gamma_uu.zz * tmp139;\
		real const tmp141 = dDelta_lll.z.xz * tmp87;\
		real const tmp142 = gamma_uu.zz * tmp141;\
		real const tmp143 = gamma_uu.yz * tmp115;\
		real const tmp144 = gamma_uu.yz * tmp116;\
		real const tmp145 = dHat_lll.z.yz * tmp87;\
		real const tmp146 = gamma_uu.zz * tmp145;\
		real const tmp147 = dDelta_lll.z.yz * tmp87;\
		real const tmp148 = gamma_uu.zz * tmp147;\
		real const tmp149 = dHat_lll.y.yz * tmp87;\
		real const tmp150 = dDelta_lll.y.yz * tmp87;\
		real const tmp151 = gamma_uu.yy * gamma_uu.yy;\
		real const tmp152 = tmp87 * tmp151;\
		real const tmp153 = gamma_uu.yz * gamma_uu.yz;\
		real const tmp154 = tmp87 * tmp153;\
		real const tmp155 = gamma_uu.yz * tmp87;\
		real const tmp156 = dHat_lll.x.xx * tmp87;\
		real const tmp157 = dDelta_lll.x.xx * tmp87;\
		real const tmp158 = gamma_uu.zz * gamma_uu.zz;\
		real const tmp159 = tmp87 * tmp158;\
		(resultFlux)->a_l.x += -1. * beta_u.x * a_l.x;\
		(resultFlux)->a_l.y += -1. * beta_u.y * a_l.x;\
		(resultFlux)->a_l.z += -1. * beta_u.z * a_l.x;\
		(resultFlux)->d_lll.x.xx += -1. * beta_u.x * dHat_lll.x.xx + -1. * beta_u.x * dDelta_lll.x.xx + -1. * beta_u.y * dHat_lll.y.xx + -1. * beta_u.z * dHat_lll.z.xx + -1. * gammaHat_ll.xx * b_ul.x.x + -1. * gammaHat_ll.xy * b_ul.y.x + -1. * gammaHat_ll.xz * b_ul.z.x + -1. * gammaDelta_ll.xx * b_ul.x.x + -1. * gammaDelta_ll.xz * b_ul.z.x + -1. * gammaDelta_ll.xy * b_ul.y.x;\
		(resultFlux)->d_lll.x.xy += -1. * beta_u.x * dDelta_lll.x.xy + tmp29 + tmp30 + tmp31 + tmp32 + tmp33 + tmp34 + tmp35 + tmp36 + tmp37 + tmp38 + tmp39 + tmp40 + tmp41 + tmp42 + tmp43;\
		(resultFlux)->d_lll.x.xz += -1. * beta_u.x * dDelta_lll.x.xz + tmp71 + tmp72 + tmp73 + tmp74 + tmp75 + tmp76 + tmp77 + tmp78 + tmp79 + tmp80 + tmp81 + tmp82 + tmp83 + tmp84 + tmp85;\
		(resultFlux)->d_lll.x.yy += -1. * beta_u.x * dDelta_lll.y.xy;\
		(resultFlux)->d_lll.x.yz += -1. * beta_u.x * dDelta_lll.y.xz;\
		(resultFlux)->d_lll.x.zz += -1. * beta_u.x * dDelta_lll.z.xz;\
		(resultFlux)->d_lll.y.xx += -1. * beta_u.y * dDelta_lll.x.xx;\
		(resultFlux)->d_lll.y.xy += -1. * beta_u.y * dDelta_lll.x.xy;\
		(resultFlux)->d_lll.y.xz += -1. * beta_u.y * dDelta_lll.x.xz;\
		(resultFlux)->d_lll.y.yy += -1. * beta_u.y * dDelta_lll.y.xy + tmp29 + tmp30 + tmp31 + tmp32 + tmp33 + tmp34 + tmp35 + tmp36 + tmp37 + tmp38 + tmp39 + tmp40 + tmp41 + tmp42 + tmp43;\
		(resultFlux)->d_lll.y.yz += -1. * beta_u.y * dDelta_lll.y.xz + tmp71 + tmp72 + tmp73 + tmp74 + tmp75 + tmp76 + tmp77 + tmp78 + tmp79 + tmp80 + tmp81 + tmp82 + tmp83 + tmp84 + tmp85;\
		(resultFlux)->d_lll.y.zz += -1. * beta_u.y * dDelta_lll.z.xz;\
		(resultFlux)->d_lll.z.xx += -1. * beta_u.z * dDelta_lll.x.xx;\
		(resultFlux)->d_lll.z.xy += -1. * beta_u.z * dDelta_lll.x.xy;\
		(resultFlux)->d_lll.z.xz += -1. * beta_u.z * dDelta_lll.x.xz;\
		(resultFlux)->d_lll.z.yy += -1. * beta_u.z * dDelta_lll.y.xy;\
		(resultFlux)->d_lll.z.yz += -1. * beta_u.z * dDelta_lll.y.xz;\
		(resultFlux)->d_lll.z.zz += -1. * beta_u.z * dDelta_lll.z.xz + tmp71 + tmp72 + tmp73 + tmp74 + tmp75 + tmp76 + tmp77 + tmp78 + tmp79 + tmp80 + tmp81 + tmp82 + tmp83 + tmp84 + tmp85;\
		(resultFlux)->Theta += -1. * beta_u.x * Theta;\
		(resultFlux)->Z_l.x += -1. * Z_l.x * beta_u.x;\
		(resultFlux)->Z_l.y += -1. * Z_l.y * beta_u.x;\
		(resultFlux)->Z_l.z += -1. * Z_l.z * beta_u.x;\
		(resultFlux)->beta_u.x = 0.;\
		(resultFlux)->beta_u.y = 0.;\
		(resultFlux)->beta_u.z = 0.;\
		(resultFlux)->b_ul.x.x = -1. * beta_u.x * b_ul.x.x + -1. * beta_u.y * b_ul.x.y + -1. * beta_u.z * b_ul.x.z + -1. * dHat_lll.x.xx * tmp88 + -1. * dDelta_lll.x.xx * tmp88 + -2. * dHat_lll.x.yy * tmp90 + -2. * dHat_lll.x.zz * tmp92 + -2. * dDelta_lll.x.yy * tmp90 + -2. * dDelta_lll.x.zz * tmp92 + a_l.x * gamma_uu.xx * tmp87 + a_l.y * tmp93 + a_l.z * tmp94 + -1. * gamma_uu.xx * gamma_uu.xy * tmp95 + -1. * gamma_uu.xx * gamma_uu.xy * tmp96 + -1. * gamma_uu.xx * tmp98 + -1. * gamma_uu.xx * tmp100 + gamma_uu.xx * tmp102 + gamma_uu.xx * tmp104 + gamma_uu.xx * tmp106 + gamma_uu.xx * tmp108 + -1. * gamma_uu.xy * gamma_uu.yy * tmp109 + -1. * gamma_uu.xy * gamma_uu.yy * tmp110 + gamma_uu.xy * tmp112 + gamma_uu.xy * tmp114 + gamma_uu.xz * gamma_uu.yy * tmp115 + gamma_uu.xz * gamma_uu.yy * tmp116 + -1. * gamma_uu.xz * tmp118 + -1. * gamma_uu.xz * tmp120 + -2. * gamma_uu.xx * gamma_uu.xy * tmp121 + -2. * gamma_uu.xx * gamma_uu.xy * tmp122 + -2. * gamma_uu.xx * gamma_uu.xz * tmp123 + -2. * gamma_uu.xx * gamma_uu.xz * tmp124 + -2. * gamma_uu.xx * tmp126 + -2. * gamma_uu.xx * tmp128 + 2. * gamma_uu.xx * gamma_uu.yz * tmp129 + -2. * gamma_uu.xx * tmp131 + -2. * gamma_uu.xx * tmp133 + 2. * gamma_uu.xx * gamma_uu.yz * tmp134 + -2. * gamma_uu.xx * tmp136 + -2. * gamma_uu.xx * tmp138 + -2. * gamma_uu.xx * tmp140 + -2. * gamma_uu.xx * tmp142 + -2. * gamma_uu.xy * tmp143 + -2. * gamma_uu.xy * tmp144 + -2. * gamma_uu.xy * tmp146 + -2. * gamma_uu.xy * tmp148 + -2. * gamma_uu.xz * gamma_uu.yy * tmp149 + -2. * gamma_uu.xz * gamma_uu.yy * tmp150 + -2. * gamma_uu.xz * gamma_uu.yz * tmp111 + -2. * gamma_uu.xz * gamma_uu.yz * tmp113 + -4. * gamma_uu.xy * gamma_uu.xz * tmp134 + -4. * gamma_uu.xy * gamma_uu.xz * tmp129;\
		(resultFlux)->b_ul.x.y = 0.;\
		(resultFlux)->b_ul.x.z = 0.;\
		(resultFlux)->b_ul.y.x = -1. * beta_u.x * b_ul.y.x + -1. * beta_u.y * b_ul.y.y + -1. * beta_u.z * b_ul.y.z + -1. * dHat_lll.y.yy * tmp152 + -1. * dDelta_lll.y.yy * tmp152 + -2. * dHat_lll.y.xx * tmp90 + -2. * dHat_lll.y.zz * tmp154 + -2. * dDelta_lll.y.xx * tmp90 + -2. * dDelta_lll.y.zz * tmp154 + a_l.x * tmp93 + a_l.y * gamma_uu.yy * tmp87 + a_l.z * tmp155 + -1. * gamma_uu.xx * gamma_uu.xy * tmp156 + -1. * gamma_uu.xx * gamma_uu.xy * tmp157 + gamma_uu.xx * gamma_uu.yy * tmp95 + gamma_uu.xx * gamma_uu.yy * tmp96 + gamma_uu.xx * gamma_uu.yz * tmp97 + gamma_uu.xx * gamma_uu.yz * tmp99 + -1. * gamma_uu.xy * tmp102 + -1. * gamma_uu.xy * tmp104 + gamma_uu.xy * tmp106 + gamma_uu.xy * tmp108 + -1. * gamma_uu.yy * tmp143 + -1. * gamma_uu.yy * tmp144 + gamma_uu.yy * tmp112 + gamma_uu.yy * tmp114 + -1. * gamma_uu.yz * tmp118 + -1. * gamma_uu.yz * tmp120 + -2. * gamma_uu.xx * gamma_uu.yy * tmp121 + -2. * gamma_uu.xx * gamma_uu.yy * tmp122 + -2. * gamma_uu.xx * gamma_uu.yz * tmp123 + -2. * gamma_uu.xx * gamma_uu.yz * tmp124 + -2. * gamma_uu.xy * tmp98 + -2. * gamma_uu.xy * tmp100 + -2. * gamma_uu.xy * tmp126 + -2. * gamma_uu.xy * tmp128 + -2. * gamma_uu.xy * tmp140 + -2. * gamma_uu.xy * tmp142 + -2. * gamma_uu.xz * gamma_uu.yy * tmp129 + 2. * gamma_uu.xz * gamma_uu.yy * tmp130 + -2. * gamma_uu.xz * gamma_uu.yy * tmp132 + -2. * gamma_uu.xz * gamma_uu.yy * tmp134 + 2. * gamma_uu.xz * gamma_uu.yy * tmp135 + -2. * gamma_uu.xz * gamma_uu.yy * tmp137 + -2. * gamma_uu.xz * gamma_uu.yz * tmp105 + -2. * gamma_uu.xz * gamma_uu.yz * tmp107 + -2. * gamma_uu.yy * gamma_uu.yz * tmp149 + -2. * gamma_uu.yy * gamma_uu.yz * tmp150 + -2. * gamma_uu.yy * tmp146 + -2. * gamma_uu.yy * tmp148 + -4. * gamma_uu.xy * tmp136 + -4. * gamma_uu.xy * tmp131;\
		(resultFlux)->b_ul.y.y = 0.;\
		(resultFlux)->b_ul.y.z = 0.;\
		(resultFlux)->b_ul.z.x = -1. * beta_u.x * b_ul.z.x + -1. * beta_u.y * b_ul.z.y + -1. * beta_u.z * b_ul.z.z + -1. * dHat_lll.z.zz * tmp159 + -1. * dDelta_lll.z.zz * tmp159 + -2. * dHat_lll.z.xx * tmp92 + -2. * dHat_lll.z.yy * tmp154 + -2. * dDelta_lll.z.xx * tmp92 + -2. * dDelta_lll.z.yy * tmp154 + a_l.x * tmp94 + a_l.y * tmp155 + a_l.z * gamma_uu.zz * tmp87 + -1. * gamma_uu.xx * gamma_uu.xz * tmp156 + -1. * gamma_uu.xx * gamma_uu.xz * tmp157 + gamma_uu.xx * gamma_uu.yz * tmp95 + gamma_uu.xx * gamma_uu.yz * tmp96 + gamma_uu.xx * gamma_uu.zz * tmp97 + gamma_uu.xx * gamma_uu.zz * tmp99 + gamma_uu.xz * tmp102 + gamma_uu.xz * tmp104 + -1. * gamma_uu.xz * tmp106 + -1. * gamma_uu.xz * tmp108 + -1. * gamma_uu.yy * gamma_uu.yz * tmp109 + -1. * gamma_uu.yy * gamma_uu.yz * tmp110 + gamma_uu.yy * gamma_uu.zz * tmp115 + gamma_uu.yy * gamma_uu.zz * tmp116 + -1. * gamma_uu.yz * tmp112 + -1. * gamma_uu.yz * tmp114 + -2. * gamma_uu.xx * gamma_uu.yz * tmp121 + -2. * gamma_uu.xx * gamma_uu.yz * tmp122 + -2. * gamma_uu.xx * gamma_uu.zz * tmp123 + -2. * gamma_uu.xx * gamma_uu.zz * tmp124 + -2. * gamma_uu.xy * gamma_uu.xz * tmp95 + -2. * gamma_uu.xy * gamma_uu.xz * tmp96 + -2. * gamma_uu.xy * gamma_uu.yz * tmp101 + -2. * gamma_uu.xy * gamma_uu.yz * tmp103 + -2. * gamma_uu.xy * gamma_uu.zz * tmp129 + -2. * gamma_uu.xy * gamma_uu.zz * tmp130 + 2. * gamma_uu.xy * gamma_uu.zz * tmp132 + -2. * gamma_uu.xy * gamma_uu.zz * tmp134 + -2. * gamma_uu.xy * gamma_uu.zz * tmp135 + 2. * gamma_uu.xy * gamma_uu.zz * tmp137 + -2. * gamma_uu.xz * tmp126 + -2. * gamma_uu.xz * tmp128 + -2. * gamma_uu.xz * tmp140 + -2. * gamma_uu.xz * tmp142 + -2. * gamma_uu.yy * gamma_uu.zz * tmp149 + -2. * gamma_uu.yy * gamma_uu.zz * tmp150 + -2. * gamma_uu.yz * tmp146 + -2. * gamma_uu.yz * tmp148 + -4. * gamma_uu.xz * tmp138 + -4. * gamma_uu.xz * tmp133;\
